...
const Gingles: React.FC<GinglesProps> = ({ selectedState }) => {
  ...
  const demographicOptions: Record<
    DemographicKey,
  ...
  const processedData = useMemo(() => {
    ...
    const filteredData =
      urbanityFilter === "all"
        ...
        : rawData.filter(
            (precinct) =>
              precinct.urbanicity?.toLowerCase() ===
              urbanityFilter.toLowerCase()
          );
    ...
    const isOutlierPoint = (points: any[], point: any) => {
      ...
      const similarPoints = points.filter(
        (p) =>
          Math.abs(
            p.electionData.biden_ratio - point.electionData.biden_ratio
          ) <
            THRESHOLD / 100 ||
          Math.abs(
            p.electionData.trump_ratio - point.electionData.trump_ratio
          ) <
            THRESHOLD / 100
      );
      ...
      <HStack justify="space-between" w="100%" align="center">
        ...
        <HStack spacing={4}>
          ...
          {(dataMode === "Demographic" || dataMode === "Income/Race") && (
            <Menu>
              <MenuButton as={Button} rightIcon={<ChevronDownIcon />}>
                {selectedDemographic.slice(0,1). toUpperCase() + selectedDemographic.slice(1).toLowerCase()}
              </MenuButton>
              <MenuList>
                {Object.entries(demographicOptions).map(([key, value]) => (
                  <MenuItem
                    onClick={() =>
                      setSelectedDemographic(value.name as DemographicKey)
                    }
                  >
